...
module FACoin::fa_coin {
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires ManagedFungibleAsset {
        ...
        assert!(!primary_fungible_store::is_frozen(to, asset),0);
    ...
    public entry fun transfer(admin: &signer, from: address, to: address, amount: u64) acquires ManagedFungibleAsset {
        let asset = get_metadata();
        assert!(!primary_fungible_store::is_frozen(from, asset),1);
        assert!(!primary_fungible_store::is_frozen(to, asset),2);
    ...
    public entry fun burn(admin: &signer, from: address, amount: u64) acquires ManagedFungibleAsset {
        let asset = get_metadata();
        assert!(!primary_fungible_store::is_frozen(from, asset),3);
    ...
    public entry fun freeze_account(admin: &signer, account: address) acquires ManagedFungibleAsset {
        let asset = get_metadata();
        assert!(!primary_fungible_store::is_frozen(account, asset),4);
        let transfer_ref = &authorized_borrow_refs(admin, asset).transfer_ref;
    ...
    public fun withdraw(admin: &signer, amount: u64, from: address): FungibleAsset acquires ManagedFungibleAsset {
        let asset = get_metadata();
        assert!(!primary_fungible_store::is_frozen(from, asset),5);
        let transfer_ref = &authorized_borrow_refs(admin, asset).transfer_ref;
        let from_wallet = primary_fungible_store::primary_store(from, asset);
    ...
    public fun deposit(admin: &signer, to: address, fa: FungibleAsset) acquires ManagedFungibleAsset {
        let asset = get_metadata();
        assert!(!primary_fungible_store::is_frozen(to, asset),6);
        let transfer_ref = &authorized_borrow_refs(admin, asset).transfer_ref;
        let to_wallet = primary_fungible_store::ensure_primary_store_exists(to, asset);
