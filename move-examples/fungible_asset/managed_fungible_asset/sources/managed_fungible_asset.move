...
module example_addr::managed_fungible_asset {
    ...
    public fun initialize(
        ...
        primary_fungible_store::create_primary_store_enabled_fungible_asset(
            constructor_ref, // create_name_object need the signer address and the coin symbol
            supply,
            name,
            symbol,
            decimals,
    ...
    public entry fun mint_to_primary_stores(
        ...
        let receiver_primary_stores = vector::map(
            to,
            |addr| primary_fungible_store::ensure_primary_store_exists(addr, asset) // make sure every address has MEtadata resources(name symbol decimal icon data-uri)
        );
    ...
    fun create_test_mfa(creator: &signer): Object<Metadata> {
        ...
        initialize(
            constructor_ref,
            0, //supply 0 means no limitation
