...
//1. Two People A and B
//2. A generate a new account IA
//3. IA has resources. IA has structs including admin account A and Vector approved account[].
//4. A could add account to IA Vector and remove it from IA Vector
//5. If B is in the IA Vector, B could ask resource from IA
module common_account::common_account {
    ...
    public entry fun create(sender: &signer, seed: vector<u8>) { //(vector[0u8, 1u8, 2u8]: vector<u8>);
    ...
    public entry fun add_account(
        sender: &signer,  // the signer account in the create call.
        common_account: address, // the IA account from the admin account
        other: address,
    ) acquires Management {
    ...
    public fun test_end_to_end(
        ...
        debug::print(&alice_addr);
        debug::print(&bob_addr);
